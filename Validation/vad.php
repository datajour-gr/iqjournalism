...
if (isset($_POST['text'])) {
    ...
    $filename = 'vad/data_' . $_GET['arthro'] . '_validate.json';
...
if (isset($_GET['arthro'])) {
    $file_name  = 'vad/data_' . $_GET['arthro'] . '.json';
    $json_data = file_get_contents($file_name);
    //if file exists check if validate file exists
    $validation_file =  'vad/data_' . $_GET['arthro'] . '_validate.json';
// var_dump($json_data);
...
$svg_names = ['valence', 'arousal', 'dominance'];
$svg = [
    '<span title="valence" class="emoji valence">&#128548;</span>',
    '<span title="arousal" class="emoji arousal">&#128559;</span>',
    '<span title="dominance" class="emoji dominance">&#128534;</span>',
];
...
<body>
    ...
    <script>
        ...
        $('.correct').on('click', function(event) {
            ...
            $.ajax({
                url: "vad.php?arthro=<?php echo $_GET['arthro']; ?>",
                method: "POST",
                data: {
                    text: row_text,
                    label: 'correct',
